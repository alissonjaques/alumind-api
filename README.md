...
Seria preciso analisar cada nova funcionalidade sugerida verificando se já tem o código cadastrado no banco de dados, no caso de ocorrer seria preciso alinhar com o time o requisito de ‘o que fazer com o reason dessa nova funcionalidade’ (ignorar, gerar um novo prompt que mescla o que tem no banco de dados com o que está vindo, etc) e, por fim, vincular a funcionalidade sugerida que já está cadastrada com o novo feedback. Nesse caso o custo e seria um pouco maior, bem como a performance do sistema por ter duas requisições à API da OpenAI por requisição à rota da AluMind, isso se o requisito exigir a criação do novo prompt.</p>
